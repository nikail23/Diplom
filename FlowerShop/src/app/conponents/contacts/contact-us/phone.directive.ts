  ...
  onInputChange(event: any, backspace: boolean) {
    if (this.ngControl.valueAccessor) {
      ...
      if (newValue.length === 0) {
        ...
        newValue = newValue.replace(/^(\d{0,3})(\d{0,2})(\d{0,3})/, '+$1 $2 $3');
      } else if (newValue.length <= 10) {
        newValue = newValue.replace(/^(\d{0,3})(\d{0,2})(\d{0,3})(\d{0,2})/, '+$1 $2 $3-$4');
      } else if (newValue.length <= 12) {
        newValue = newValue.replace(/^(\d{0,3})(\d{0,2})(\d{0,3})(\d{0,2})(\d{0,2})/, '+$1 $2 $3-$4-$5');
      }
      else {
